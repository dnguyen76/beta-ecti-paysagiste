...
export interface MapViewProps {
  ...
  onGetViewBboxRef?: React.MutableRefObject<(() => [number,number,number,number] | null) | null>;
...
export default function MapView({
  ...
  useEffect(() => {
    ...
    // Exposer une fonction pour récupérer la bbox visible en WGS84
    if (onGetViewBboxRef) {
      onGetViewBboxRef.current = (): [number,number,number,number] | null => {
        const m = mapInstance.current;
        if (!m) return null;
        const size = m.getSize();
        if (!size) return null;
        const ext = m.getView().calculateExtent(size); // [xMin, yMin, xMax, yMax] WebMercator
        const sw = toLonLat([ext[0], ext[1]]);
        const ne = toLonLat([ext[2], ext[3]]);
        return [sw[0], sw[1], ne[0], ne[1]]; // [lonMin, latMin, lonMax, latMax]
      };
    }
